...
class CardPrintWizard(models.TransientModel):
    ...
    def get_template(self):
        card_template = self.env['card.template'].search([('card_model', '=', self._context.get('active_model'))],
                                                         limit=1)
        return card_template

    template_id = fields.Many2one('card.template', 'Card Template', required=1, ondelete='cascade',
    ...
    def _preview_body(self):
        ...
        if len(res_ids) == 1:
            template = self.env['card.template'].render_template(self.template_id.body_html, model, res_ids)
