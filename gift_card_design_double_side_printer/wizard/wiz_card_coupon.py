        ...
        for rec in self:
            ...
            for i, coupon in enumerate(self.env['product.coupon'].browse(context.get('active_ids'))):
                ...
                if rec.template_id.print_data_type == 'path':
                    index, print_data = rec.template_id.create_json_duplex_data(front_path, back_path, coupon.name)
                else:
    ...
    def print_nondouplex(self):
        ...
        for rec in self:
            ...
            for i, coupon in enumerate(self.env['product.coupon'].browse(context.get('active_ids'))):
                ...
                if rec.template_id.print_data_type == 'path':
                    index, print_data = rec.template_id.create_json_nonduplex_front_data(front_path, coupon.name)
                else:
...
class WizardnondupluexPrint(models.TransientModel):
    ...
    def print_data_front(self):
        ...
        for rec in self:
            ...
            if rec.template_id.print_data_type == 'path':
                index, print_data = rec.template_id.create_json_nonduplex_front_data(front_path, rec.coupon_id.name)
            else:
